...
public class PetriObjModel implements Serializable, Cloneable  {
    ...
    public double go(double timeModeling, double epsilon) {
        double min;
        this.setSimulationTime(timeModeling);   
        this.setCurrentTime(0.0); 
        // for unstable period calculation
        double meanMarkersBeginInterval;
        double delta = Double.MAX_VALUE;
        ArrayList<Boolean> stableInRow = new ArrayList<>();
        stableInRow.add(true);
        int stable = 0;
        ...
        while (this.getCurrentTime() < this.getSimulationTime() && stable<5) { 

            meanMarkersBeginInterval = this.getListObj().get(0).getNet().getListP()[11].getMean();
            conflictObj.clear();
           ...
           this.setCurrentTime(min); // просування часу //3.12.2015
            ...
            delta = Math.abs(this.getListObj().get(0).getNet().getListP()[11].getMean()-meanMarkersBeginInterval);
            int printTime = (int) this.getCurrentTime();
            if (printTime%50==0){
                System.out.println("Mean in position Вільний 5: " + this.getListObj().get(0).getNet().getListP()[11].getMean() + " Delta: " + delta + " Time: " + this.getCurrentTime());
            } 
            outputIndex++;
            if (delta<epsilon && delta!=0)
                stableInRow.add(true);
            else
                stableInRow.add(false);
                        
            if (delta<epsilon && delta!=0 && stableInRow.get(outputIndex-1))
                stable++;
            else
                stable = 0;
        }
